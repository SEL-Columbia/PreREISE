...
class scenarios:
    ...
    def __init__(self, id, input_series, other = None):
        ...
        if other == None:
            self.stats = input_series.dropna() #dropna is supposed to drop rows that only used for defining future scenarios 
    ...
    def _compute_base_scenario(self):
        ...
        self.hp_heat_area_m2 = (self.stats['frac_hp_res_heat'] * self.floor_area_m2["res"] +
                                self.stats['frac_hp_com_heat'] * self.floor_area_m2["com"])
        self.hp_heat_frac = self.hp_heat_area_m2 / (self.floor_area_m2["res"] + self.floor_area_m2["com"])
        self.resist_heat_area_m2 = (self.stats['frac_resist_res_heat'] * self.floor_area_m2["res"] +
                                    self.stats['frac_resist_com_heat'] * self.floor_area_m2["com"])
        self.resist_heat_frac = self.resist_heat_area_m2 / (self.floor_area_m2["res"] + self.floor_area_m2["com"])
        self.elec_cool_m2 = (self.stats['frac_elec_res_cool'] * self.floor_area_m2["res"] +
                            self.stats['frac_elec_com_cool'] * self.floor_area_m2["com"])
        return

    def _compute_project_scenario(self, other):
        self.pop = other.pop * (1 + self.stats['pop_ann_grow_rate']) ** (self.year - other.year)     
        self.floor_area_m2 = {}
        for clas in const.classes:
            if np.isnan(self.stats[f'{clas}_area_ann_grow_rate']):
                ...
                self.floor_area_m2[clas] = other.floor_area_m2[clas] * (1 + self.stats['pop_ann_grow_rate']) ** (self.year - other.year)
            else:
                self.floor_area_m2[clas] = other.floor_area_m2[clas] * (1 + self.stats[f'{clas}_area_ann_grow_rate']) ** (self.year - other.year)
            ...
            if not np.isnan(self.stats[f'frac_hp_{clas}_heat']):
                ...
                self.stats[f'ff2hp_{clas}'] = 1 - self.stats[f'frac_ff_{clas}_heat'] / other.stats[f'frac_ff_{clas}_heat']
            elif not np.isnan(self.stats[f'ff2hp_{clas}']):
                self.stats[f'frac_hp_{clas}_heat'] = (other.stats[f'frac_hp_{clas}_heat'] + 
                                                        other.stats[f'frac_resist_{clas}_heat'] * self.stats[f'resist2hp_{clas}'] +
                                                        other.stats[f'frac_ff_{clas}_heat'] * self.stats[f'ff2hp_{clas}'])
                self.stats[f'frac_resist_{clas}_heat'] = other.stats[f'frac_resist_{clas}_heat'] * (1 - self.stats[f'resist2hp_{clas}'])
                self.stats[f'frac_ff_{clas}_heat'] = other.stats[f'frac_ff_{clas}_heat'] * (1 - self.stats[f'ff2hp_{clas}'])
            else:
                self.stats[f'frac_hp_{clas}_heat'] = other.stats[f'frac_hp_{clas}_heat']
                self.stats[f'frac_resist_{clas}_heat'] = other.stats[f'frac_resist_{clas}_heat']
            ...
            if np.isnan(self.stats[f'frac_elec_{clas}_cool']):
                self.stats[f'frac_elec_{clas}_cool'] = other.stats[f'frac_elec_{clas}_cool']
            ...
            if np.isnan(self.stats[f'frac_ff_{cook_other}_{clas}']):
                self.stats[f'frac_ff_{cook_other}_{clas}'] = other.stats[f'frac_ff_{cook_other}_{clas}']    
    ...
    def floor_area_growth(self, other):
        ...
        return (self.floor_area_m2["res"] + self.floor_area_m2["com"]) / (other.floor_area_m2["res"] + other.floor_area_m2["com"])
    ...
    def frac_cook_ff2hp(self, other, clas):
        ...
        cook_other = "cook" if clas == "com" else "other"
        return (other.stats[f"frac_ff_{cook_other}_{clas}"] - self.stats[f"frac_ff_{cook_other}_{clas}"])
...
def temp_to_energy(temp_series, hourly_fits_df, db_wb_fit, base_scen, hp_heat_cop):
    ...
    if temp <= t_bph:
        ...
        hp_eng = heat_eng * (base_scen.hp_heat_frac/cop_hp) / (base_scen.resist_heat_frac + base_scen.hp_heat_frac/cop_hp)
        resist_eng = heat_eng * (base_scen.resist_heat_frac) / (base_scen.resist_heat_frac + base_scen.hp_heat_frac/cop_hp)
...
def scale_energy(base_energy, temp_df, base_scen, new_scen, midperfhp_cop, advperfhp_cop):
    ...
    cool_load_scaler = new_scen.frac_cooling_eff_change(base_scen) * new_scen.frac_cool_growth(base_scen)
    ...
    if new_scen.compare_hp_heat_type(base_scen):
        ...
        for i in temp_df.index:
            hp_cop_scaler.loc[i] = hp_cop_adv.loc[round(temp_df.loc[i, 'temp_c'], 1), 'cop']
    ...
    if new_hp_profile == 'elec':  # if user select to use the current electricity heat pump consumption profiles for heating load projection
        scen_load_MWh['heat_hp_load_mw'] = base_energy['heat_hp_load_mw'] * hp_load_scaler
    else:
        scen_load_MWh['heat_existing_hp_load_mw'] = base_energy['heat_hp_load_mw']
    scen_load_MWh['heat_resist_load_mw'] = base_energy['heat_resist_load_mw'] * resist_load_scaler
...
def ff_electrify_profiles(weather_years, puma_data, new_scen, base_scen):
    ...
    def ff2hp_dhw_profiles(clas):
        ...
        for (weather_year,state) in product(weather_years, zone_states):
            if not os.path.isfile(os.path.join(os.path.dirname(__file__), "Profiles", f"elec_dhw_ff2hp_{clas}_{state}_{weather_year}_{hp_type_dhw}_mw.csv")):
                ...
                list(
                    pd.Series(data=zone_states).apply(
                        lambda x: pd.read_csv(
                            os.path.join(
                                os.path.dirname(__file__), "Profiles", f"elec_dhw_ff2hp_{clas}_{x}_{weather_year}_{hp_type_dhw}_mw.csv"
                            )
                        )
                    )
                ), axis=1
            )
            ff2hp_dhw_pumas_yrs = pd.concat([ff2hp_dhw_pumas_yrs, ff2hp_dhw_pumas], ignore_index=True)
        ff2hp_dhw_yrs = ff2hp_dhw_pumas_yrs[puma_data.index].mul(puma_data['frac_in_zone']).sum(axis=1)
        return ff2hp_dhw_yrs

    def ff2hp_htg_profiles(clas):
        ff2hp_htg_pumas_yrs = pd.DataFrame()
        for (weather_year,state) in product(weather_years, zone_states):
            if not os.path.isfile(os.path.join(os.path.dirname(__file__), "Profiles", f"elec_htg_ff2hp_{clas}_{state}_{weather_year}_{hp_type_heat}_mw.csv")):
            ...
            ff2hp_htg_pumas = pd.concat(
                list(
                    pd.Series(data=zone_states).apply(
                        lambda x: pd.read_csv(
                            os.path.join(
                                os.path.dirname(__file__), "Profiles", f"elec_htg_ff2hp_{clas}_{x}_{weather_year}_{hp_type_heat}_mw.csv"
                            )
                        )
                    )
                ), axis=1
            )
            ff2hp_htg_pumas_yrs = pd.concat([ff2hp_htg_pumas_yrs, ff2hp_htg_pumas], ignore_index=True)
        ff2hp_htg_yrs = ff2hp_htg_pumas_yrs[puma_data.index].mul(puma_data['frac_in_zone']).sum(axis=1)
        return ff2hp_htg_yrs

    def ff2hp_cook_profiles(clas):
        for state in zone_states:
            if not os.path.isfile(os.path.join(os.path.dirname(__file__), "Profiles", f"elec_cook_ff2hp_{clas}_{state}_{base_year}_{cook_eff}_mw.csv")):
        ...
        ff2hp_cook_pumas = pd.concat(
            list(
                pd.Series(data=zone_states).apply(
                    lambda x: pd.read_csv(
                        os.path.join(
                            os.path.dirname(__file__), "Profiles", f"elec_cook_ff2hp_{clas}_{x}_{base_year}_{cook_eff}_mw.csv"
                        ), index_col=0
                    )                
                )
            )
        ).iloc[:,0]
        ff2hp_cook = ff2hp_cook_pumas.loc[puma_data.index].mul(puma_data['frac_in_zone']).sum()
        return ff2hp_cook

    zone_states = list(set(puma_data["state"]))
    hours_utc_weather_years = pd.date_range(
        start=f"{weather_years[0]}-01-01", end=f"{weather_years[-1]+1}-01-01", freq="H", tz="UTC"
    )[:-1]
...
def predict_scenario(zone_name, zone_name_shp, base_scen, new_scens, weather_years):
    ...
    t_bpc_start = 10
    db_wb_regr_df = temp_df[(temp_df.index.year == base_year) & (temp_df["temp_c"] >= t_bpc_start)]
    db_wb_fit = np.polyfit(db_wb_regr_df["temp_c"], db_wb_regr_df["temp_c_wb"], 2)

    hourly_fits_df = pd.read_csv(
        f"https://besciences.blob.core.windows.net/datasets/bldg_el/dayhour_fits/{zone_name}_dayhour_fits_{base_year}.csv", index_col=0
    )
    ...
    for id, scenario in new_scens.items(): 
        elec_profile_load_MWh[id] = scale_energy(zone_profile_refload_MWh, temp_df, base_scen, scenario, midperfhp_cop, advperfhp_cop)
        ff2hp_profile_load_MWh[id] = ff_electrify_profiles(weather_years, puma_data_zone, scenario, base_scen)
        zone_profile_load_MWh[id] = pd.concat([elec_profile_load_MWh[id], ff2hp_profile_load_MWh[id]], axis=1)
...
if __name__ == "__main__":
    ...
    for i in range(len(zone_names)):
        ...
        zone_profile_load_MWh = predict_scenario(zone_name, zone_name_shp, base_scenarios, proj_scenarios, weather_years)
        
        
        os.makedirs(os.path.join(os.path.dirname(__file__), "projection", "results"), exist_ok=True)
